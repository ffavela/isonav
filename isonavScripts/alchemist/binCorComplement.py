...
ring_tags=["1i", "1e", "2i", "2e", "3i", "3e", "4i", "4e", "5i", "5e",
           "6i", "6e", "7i", "7e", "8i", "8e", "9i", "9e", "S10",
           "S11", "S12","S13", "S14", "S15", "S16", "S17", "S18",
           "S19", "S20", "S21", "S22","S23", "S24", "S25", "S26"]

#The distance from the sphere center to the detectors (cm)
det_dist=[350, 350, 300, 300, 250, 250, 210, 210, 180, 180, 160, 160,
          140, 140, 120, 120, 100, 100, 40, 40, 40, 40, 40, 40, 40,
...
firstTelL=[0, 16, 32, 56, 80, 112, 144, 184, 224, 264, 304, 352, 400,
	   448, 496, 544, 592, 640, 688, 720, 752, 784, 816, 848, 880,
	   912, 944, 976, 1008, 1040, 1072, 1104, 1136, 1168, 1184]

lastTelL=[15, 31, 55, 79, 111, 143, 183, 223, 263, 303, 351, 399, 447,
	  495, 543, 591, 639, 687, 719, 751, 783, 815, 847, 879, 911,
	  943, 975, 1007, 1039, 1071, 1103, 1135, 1167, 1183, 1191]

dOmega=[0.133, 0.209, 0.247, 0.326, 0.373, 0.458, 0.549, 0.660, 0.756,
        0.870, 0.813, 0.902, 1.337, 1.485, 1.639, 1.785, 2.950, 3.270,
        15.313, 18.313, 21.000, 23.250, 25.063, 26.375, 27.125,
...
vector
    ...
    idx2=subDect
    theta=radians(thetaVals[idx1])
    phi=radians(delta_phi[idx1]*idx2)
...
def getThetaRel(rNorm1,rNorm2):
    """Gets the relative thetaRel angle, for two normalized vectors, via the
dot product"""
...
detector"""
    ...
    for i in range(len(ring_tags)):
        ...
        for myInternalIdx in range(teles_num[i]):
            ...
            myThetaDict[iTag].append(degrees(thetaRel))
